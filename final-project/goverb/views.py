...
def verbs_update(request):
    ...
    if request.method == "GET":
        ...
        if type == "regular":
            ...
            # If user dont have enough verbs 
            if len(less_scored_verbs) < 10:
                # Take random 10 from verbs db
                v = Verb.objects.order_by('?')[:10]
                verbs = []
                for verb in v:
                    verbs.append(verb.forms.split(","))
            ...
            if user_verbs.count() > 110:
                ...
                v = Verb.objects.filter(id__in=verbs_id_by_repeat).order_by('?')[:10]
                 ...
                 # Else take all user verbs id and exclude from lis
    ...
    if request.method == "PUT":
        ...
        count =  80 + 75 / score.repeat - data.get("answer")["hint"] / 0.04 - data.get("answer")["tries"] / 0.1 -  data.get("answer")["hint"] / 0.04
        if count < 0:
            count = 0
        score.score = count
        score.save()
